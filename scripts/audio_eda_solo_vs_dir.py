...
FILE_B = "../data/all_sorted/BIC"   # dataset koji usporedujemo

LABEL_A = "Čisti bicikl" 
LABEL_B = "Cijeli dataset bicikala"


OUTPUT_DIR = Path("../eda_analiza_solo_vs_dir_comparison")
SR_TARGET  = 48000
N_MFCC     = 13
DURATION   = None
...
def load_both():
    ...
    # =cisti bicikl
    fpath_A = Path(FILE_A)
    if not fpath_A.exists():
        raise FileNotFoundError(f"File nije pronađen: {FILE_A}")
    yA, srA = librosa.load(str(fpath_A), sr=SR_TARGET, mono=True, duration=DURATION)

    results[LABEL_A] = {"y": yA, "sr": srA, "name": fpath_A.name, "path": FILE_A}
    durA = librosa.get_duration(y=yA, sr=srA)
    rmsA = float(np.sqrt(np.mean(yA**2)))
    print(f"  [{LABEL_A}]  {fpath_A.name}")
    print(f"    Trajanje: {durA:.3f}s | RMS: {rmsA:.6f} | SR: {srA}Hz")


    # B nam je sada dataset
    folder = Path("../data/all_sorted/BIC/")
    if not folder.exists():
        raise FileNotFoundError(f"Folder nije pronađen: {folder}")
    ...
    for f in folder.glob("*.wav"):
        try:
            y, sr = librosa.load(str(f), sr=SR_TARGET, mono=True, duration=DURATION)
            all_audio.append(y)
            file_count += 1
        except Exception as e:
            print(f"  ⚠ preskočen fajl: {f.name}")

    if len(all_audio) == 0:
        raise ValueError("Nema učitanih audio fajlova u datasetu.")

    # sve u jedan signal
    yB = np.concatenate(all_audio)

    # ili ako zelimo vise statiscki fer onda mozemo uzimat i rposjecni bicikl
    """min_len = min(len(y) for y in all_audio)
    all_audio = [y[:min_len] for y in all_audio]
    yB = np.mean(all_audio, axis=0)"""
    

    results[LABEL_B] = {
        "y": yB,
        "sr": SR_TARGET,
        "name": f"{file_count} files concatenated",
        "path": str(folder)
    }

    durB = librosa.get_duration(y=yB, sr=SR_TARGET)
    rmsB = float(np.sqrt(np.mean(yB**2)))

    print(f"  [{LABEL_B}]  DATASET ({file_count} fajlova)")
    print(f"    Trajanje: {durB:.3f}s | RMS: {rmsB:.6f} | SR: {SR_TARGET}Hz")
